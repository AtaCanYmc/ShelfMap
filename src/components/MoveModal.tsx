...
export const MoveModal: FC<MoveModalProps> = ({
  ...
  const title = targetItem
    ? `${t('relocateTitleItem')}: ${targetItem.name}`
    : `${t('relocateTitleContainer')}: ${targetContainer?.name}`
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-[11px] text-slate-500 tabular-nums">
              {validContainers.length + 1} {t('targetsAvailable')}
            </span>
          </div>
          ...
          <div className="max-h-64 overflow-y-auto space-y-1.5 pr-1 font-mono text-xs">
            ...
            >
              ...
              {selectedId === null && <Check className="w-4 h-4 text-amber-500 shrink-0" />}
            </button>
          ...
          <div className="flex items-center justify-end gap-2.5 pt-3 border-t border-slate-200 dark:border-[#232a3c]">
            ...
            <button
              ...
              onClick={handleMove}
              disabled={loading}
              className="flex items-center gap-2 px-5 py-2 rounded-lg bg-amber-500 hover:bg-amber-400 disabled:opacity-50 text-black text-xs font-semibold uppercase tracking-wider transition-colors shadow-sm min-h-[38px]"
            >
              {loading && <Loader2 className="w-3.5 h-3.5 animate-spin text-black" />}
              <span>{t('confirmMoveBtn')}</span>
            </button>
          </div>
        </div>
      </div>
